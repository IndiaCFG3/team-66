var MissingDocs = function (User, Scheme) {
  ...
  for (var doc_name in Scheme.docs) {
    if (Scheme.docs[doc_name] == 1) {
      //This doc is required
...
var GenerateList = function (User, Schemes) {
  ...
  for (var Scheme in Schemes) {
    if (
      isIncomeEligible(User, Scheme) &&
      isSameLocation(User, Scheme) &&
      isAgeEligible(User, Scheme) &&
      isGenderEligible(User, Scheme)
    ) {
...
module.exports.isSameLocation = inSameLocation;
